		...
		BOND * bond, CONTROLINFO *cinfo, MOLINFO *minfo)
{
	...
	if ((fpin = fopen(filename, "r")) == NULL) {
		fprintf(stderr, "Cannot open the ac input file: %s, exit\n", filename);
		exit(1);
	}
...
/* bond type is frozen during bond type assignment */
				...
				if(bond[numbond].jflag == 1 && bond[numbond].type == 10) {
					fprintf(stderr, "You cannot freeze this bond (%s-%s,%d) since it is unclear whether it is a single or double bond\n",atom[bond[numbond].bondi].name, atom[bond[numbond].bondj].name, bond[numbond].type); 
					fprintf(stderr, "Try to use '7' for aromatic single and '8' for aromatic double bond, exit\n"); 
					exit(1);
				}
				if (tmpint3 == 0)
					(*cinfo).bpindex = 1;
		 ...
		 BOND * bond, CONTROLINFO cinfo, MOLINFO minfo)
{
	...
	if ((fpout = fopen(filename, "w")) == NULL) {
		fprintf(stderr, "Cannot open a file (%s) to write in ac format, exit\n", filename);
		exit(1);
	}
