...
C
      ...
      character(len=4) katn, ifind, nfind, iiend, ksear, ihol, ktypg
...
C
      character(len=4) jgraph,jresnm,jsymbl,jtree
      COMMON/PROPF/JGRAPH(11),JRESNM(11),JSYMBL(11),JTREE(11),ISRCH
      character(len=4) lbres,itree,igraph,isymbl,iwild,jwild
...
C
      DIMENSION IPRES(*)
C
      IF(NUMA.LE.0) GO TO 11
      IF(NUMA.GE.IPRES(NRES)) IRES = NRES
      IF(NUMA.GE.IPRES(NRES)) RETURN
      IM = NRES-1
      DO 10 I = 1,IM
        IF(NUMA.GE.IPRES(I).AND.NUMA.LT.IPRES(I+1)) GO TO 12
   10 CONTINUE
   11 WRITE(6,100) NUMA
      GO TO 200
   12 IRES = I
