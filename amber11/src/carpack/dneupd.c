/* dneupd.f -- translated by f2c (version 20030306).
...
/*        %-------------------------------------------% */
	...
	if (s_cmp(which, "LM", (ftnlen)2, (ftnlen)2) == 0 || s_cmp(which, 
	    ...
	    thres = dlapy2_(&workl[ritzr], &workl[ritzi]);
...
/*        %----------------------------------------------------------% */
	...
	for (j = 0; j <= i__1; ++j) {
		...
		if (dlapy2_(&workl[irr + j], &workl[iri + j]) >= thres) {
/* Computing MAX */
		    d__1 = eps23, d__2 = dlapy2_(&workl[irr + j], &workl[iri 
			    + j]);
		    temp1 = max(d__1,d__2);
...
/*        %-----------------------------------------------------------% */
	...
	dlaset_("All", ncv, ncv, &c_b44, &c_b45, &workl[invsub], &ldq, (
		ftnlen)3);
...
/*           %---------------------------------------------------% */
		...
		dscal_(&nconv, &c_b71, &workl[iuptri + j - 1], &ldq);
		dscal_(&nconv, &c_b71, &workl[iuptri + (j - 1) * ldq], &c__1);
	    }
...
/* L40: */
	    ...
	    dgemv_("T", ncv, &nconv, &c_b45, &workl[invsub], &ldq, &workl[
...
/*        %---------------------------------------% */
	...
	dger_(n, &nconv, &c_b45, &resid[1], &c__1, &workev[1], &c__1, &z__[
		z_offset], ldz);
