/* dseupd.f -- translated by f2c (version 20030306).
...
/* \SCCS Information: @(#) */
/* FILE: seupd.F   SID: 2.8   DATE OF SID: 5/19/98   RELEASE: 2 */
...
/*        %------------------------------------------------% */
	...
	if (s_cmp(which, "LM", (ftnlen)2, (ftnlen)2) == 0 || s_cmp(which, 
	    ...
	    if (msglvl > 2) {
		dvout_(&debug_1.logfil, &c__1, &thres1, &debug_1.ndigit, 
...
/*            %------------------------------------------------% */
		...
		kv[0] = thres1;
		kv[1] = thres2;
		dvout_(&debug_1.logfil, &c__2, kv, &debug_1.ndigit, "_seupd:"
...
/* L120: */
    ...
    if (s_cmp(type__, "REGULR", (ftnlen)6, (ftnlen)6) != 0) {
	dger_(n, &nconv, &c_b119, &resid[1], &c__1, &workl[iw], &c__1, &z__[
		z_offset], ldz);
    }
