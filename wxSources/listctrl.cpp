...
//  Define an event class to implement delayed syncing of the header window
wxDECLARE_EVENT(ListCtrlEvent, wxCommandEvent);
wxDEFINE_EVENT(ListCtrlEvent, wxCommandEvent);
//  End Toshi Nagata
...
//  2013.12.10. Toshi Nagata; include my own version of private/listctrl.h
//#include "wx/generic/private/listctrl.h"
#include "listctrl_private.h"
//  End Toshi Nagata
    ...
    #include "wx/msw/wrapwin.h"   // 2013.11.27. define -> include Toshi Nagata
#endif
...
//  2013.12.30. Toshi Nagata
#elif defined(__WXMSW__)
static const int EXTRA_HEIGHT = 1;
//  End Toshi Nagata
#else
...
void wxListLineData::ApplyAttributes(wxDC *dc,
                                     const wxRect& rectHL,
                                     bool highlighted,
                                     bool current, wxListItemAttr *override) // 2013.12.10. Toshi Nagata; add "override" argument
{
	//  2013.12.10. Toshi Nagata
//    const wxListItemAttr * const attr = GetAttr();
	const wxListItemAttr * const attr = (override ? override : GetAttr());
	//  End Toshi Nagata
    ...
    if ( highlighted )
    {
		...
		//  Modified Toshi Nagata 2013.12.30.
...
void wxListLineData::DrawInReportMode( wxDC *dc,
                                       ...
                                       const wxRect& rectHL,
                                       bool highlighted,
                                       bool current, long line ) // 2013.12.10. Toshi Nagata; add "line" argument
{
...
//    ApplyAttributes(dc, rectHL, highlighted, current);  //  2013.12.10. Toshi Nagata; move to the inside the loop
          ...
          node = node->GetNext(), col++ )
    {
		...
		//  2013.12.10. Toshi Nagata
		//  Implement attribute for each column
		{
			wxListItemAttr *cattr = m_owner->GetListCtrl()->OnGetItemAttr(line + 1000000 * (col + 1));
			wxRect itemRect(xOld, rect.y, width, rect.height);
			ApplyAttributes(dc, itemRect, highlighted, current, cattr);
		}
		//  End Toshi Nagata
...
BEGIN_EVENT_TABLE(wxListHeaderWindow,wxWindow)
    ...
    EVT_COMMAND(-1, ListCtrlEvent, wxListHeaderWindow::OnListCtrlEvent)  //  Toshi Nagata 2014.01.13.
END_EVENT_TABLE()
...
void wxListHeaderWindow::QueueListCtrlEvent(wxCommandEvent &event)
{
	AddPendingEvent(event);
}

void wxListHeaderWindow::OnListCtrlEvent(wxCommandEvent &event)
{
	Refresh();
	Update();	
}
...
extern void wxListCtrl_onPaintCallback(wxGenericListCtrl *listctrl, wxDC *dc); //  2013.12.10. Toshi Nagata
...
void wxListMainWindow::OnPaint( wxPaintEvent &WXUNUSED(event) )
{
    ...
    if ( InReportView() )
    {
        ...
        for ( size_t line = visibleFrom; line <= visibleTo; line++ )
        {
            ...
            GetLine(line)->DrawInReportMode( &dc,
                                             ...
                                             IsHighlighted(line),
                                             line == m_current, line ); // 2013.12.10. Toshi Nagata; add "line" argument
        }
	...
	::wxListCtrl_onPaintCallback(GetListCtrl(), &dc);  // 2013.12.10. Toshi Nagata
