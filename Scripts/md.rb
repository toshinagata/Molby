...
#    }
  ...
  def clear_md_parameters
    if natoms == 0
      error_message_box "Molecule is empty"
      return
    end
    p = self.parameter
    [p.vdws, p.bonds, p.angles, p.dihedrals, p.impropers, p.vdw_pairs].each { |en|
...
register_menu("MM/MD\tClear MD Parameters", :clear_md_parameters, :non_empty)
register_menu("MM/MD\t-", nil)
