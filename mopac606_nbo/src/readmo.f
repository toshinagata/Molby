...
C
      BANNER=' ** [MOPAC] Ver.6 by James J. P. Stewart      '//
     1'                                **'
      WRITE(6,'(A)')BANNER
      WRITE(6,'(1X,15(''*****''),''****'')')
      BANNER=' ** PDDG modification by W. L. Jorgensen et al'//
     1'.(Yale Univ.)                   **'
      WRITE(6,'(A)')BANNER
      BANNER=' ** Additional modification by N. Senda(Tencub'//
     1'e) and W. Sasaki(pc-chem.info)  **'
      WRITE(6,'(A)')BANNER
C
...
C
      ...
      IF(INDEX(KEYWRD,'PDG')   .NE. 0) LINE='PDDG/PM3'
      WRITE(6,'(/28X,A,'' CALCULATION RESULTS'',29X,///1X,15(''*****'')
     1,''****'' )')LINE(:8)
